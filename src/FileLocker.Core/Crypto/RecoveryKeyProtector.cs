...
/// <summary>
/// 對應規格文件「恢復金鑰」設計：密碼、Passkey 之外的第三條解鎖路徑，給密碼忘記、Passkey 又剛好在
/// 這台裝置上用不了的情境當備援。跟 Passkey 用同一套「衍生包裝金鑰 → 包住內容金鑰」的邏輯，
/// 差別只在金鑰來源——不是 Windows Hello 簽章，是啟用當下產生的一組高熵值隨機碼，
/// 使用者自己抄下來保管，FileLocker 不會留下任何副本。
///
/// 恢復金鑰本身就是另一把能完全解密內容的鑰匙，安全性等同密碼——外洩的風險要用管理密碼的
/// 謹慎程度去看待，這點要在 GUI 顯示時明確告知使用者。
/// </summary>
public static class RecoveryKeyProtector
{
    ...
    /// <summary>用包裝金鑰把內容金鑰包起來，回傳可以直接存進 .meta.json 的 Base64 字串。跟 PasskeyProtector.WrapContentKey 邏輯完全一樣。</summary>
    ...
    /// <summary>解開存在 .meta.json 裡的包裝內容金鑰。恢復金鑰錯誤會丟出 CryptographicException。</summary>
